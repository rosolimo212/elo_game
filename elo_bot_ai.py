...
# battle log
async def make_battle_log(message, battle1, battle2, result):
    user = message.from_user.username
    user_id = message.from_user.id
...
async def make_ratings(battle1, battle2, result):
...
async def choise_category(bot, message):
...
async def start_game(bot, message, topic):
    items = dl.get_data("""
                    select item_name
                    from tl.items
                    where {slice} = 1
...
async def skip_game(bot, message):
    markup = make_answer_buttons([
         'Продолжить',
...
async def finish_game(bot, message, battle1, battle2, result):
...
async def launch_ratings(bot, message):
...
Чем больше число (рейтинг), тем чаще игроки выбирали эту книгу
    ...
    file = open('hm.png', 'rb')
    await bot.send_photo(message.chat.id, file, reply_markup=markup)
       
async def launch_personal_stat(bot, message):
    user = message.from_user.username
    user_id = message.from_user.id
...
async def launch_leaderboard(bot, message):
...
async def handle_text(message):
    ...
    if message.text.strip() in 'Начать игру':
        ...
        topic = message.text
        try:
            topic, battle1, battle2 = await start_game(bot, message, topic)
        except:
            await launch_404(bot, message)
        ...
        try:
            if (message.text.strip() == battle1):
                ...
                result = -1
            await finish_game(bot, message, battle1, battle2, result)
        except Exception as e:
            print(str(e))
            await launch_404(bot, message)
    elif message.text.strip() in ['Пропустить']:
        await skip_game(bot, message)
    else:
        try:
            await launch_404(bot, message)
        except:
